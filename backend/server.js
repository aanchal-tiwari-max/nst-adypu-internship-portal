...
async function load_data() {
  try {
    let gcred_str = process.env.GOOGLE_CREDENTIALS??"";

    if (!gcred_str) {
      gcred_str = await fs.readFile(
    ...
    const response1 = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: "Resume Approval!A1:AI999", // chansge as needed
    });
...
function computeStats(data) {
  ...
  const entrepreneurship = data.filter(
    (s) => s.status === "entrepreneurship",
  ).length;
  const alreadyWorking = data.filter(
    (s) => s.status === "Already working",
  ).length;
  const internalCP = data.filter(
    (s) => s.status === "Internal Placed CP",
  ).length;
  const externalOffer = data.filter(
    (s) => s.status === "External Offer",
  ).length;
  const backToPlacement = data.filter(
    (s) => s.status === "Back to Placement",
  ).length;
  const resumeApproved = data.filter((s) => s.resume === "Approved").length;
  const resumeCorrection = data.filter(
    (s) => s.resume === "Correction Required",
  ).length;
  const resumeNotRequired = data.filter(
    (s) => s.resume === "Resume Not required",
  ).length;
  ...
  return {
    ...
    approvalRate:
      total > 0 ? ((resumeApproved / total) * 100).toFixed(1) : "0.0",
  };
}
...
app.get("/api/mentors/:name", (req, res) => {
  ...
  const correction = students.filter(
    (s) => s.resume === "Correction Required",
  ).length;
